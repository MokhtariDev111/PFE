...
interface OrbProps {
  ...
  beating?: boolean; // pulse like a heartbeat when true
}
...
export default function Orb({
  ...
  useEffect(() => { beatRef.current = beating; }, [beating]);
  ...
  const frag = /* glsl */ `
    ...
    vec4 draw(vec2 uv) {
      ...
      darkCol=(darkCol+v1)*v2*v3; darkCol=clamp(darkCol,0.0,1.0);
      vec3 lightCol=(colBase+v1)*mix(1.0,v2*v3,fadeAmount);
      lightCol=mix(backgroundColor,lightCol,v0); lightCol=clamp(lightCol,0.0,1.0);
      return extractAlpha(mix(darkCol,lightCol,bgLum));
    }
    vec4 mainImage(vec2 fragCoord) {
      vec2 center=iResolution.xy*0.5;
      float size=min(iResolution.x,iResolution.y);
      vec2 uv=(fragCoord-center)/size*2.0*beatScale;
  ...
  useEffect(() => {
    ...
    const update = (t: number) => {
      ...
      program.uniforms.hover.value          += (0 - program.uniforms.hover.value) * 0.1;

      // Heartbeat: scale pulses between 1.0 and 0.88 at ~1.2 Hz
      if (beatRef.current) {
        const beat = 1.0 - 0.12 * Math.abs(Math.sin(t * 0.0038));
        program.uniforms.beatScale.value += (beat - program.uniforms.beatScale.value) * 0.18;
      } else {
        program.uniforms.beatScale.value += (1.0 - program.uniforms.beatScale.value) * 0.1;
      }
