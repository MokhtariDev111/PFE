...
def extract_section_outline(chunks: list) -> list[str]:
    ...
    Falls back to the chunk's first meaningful line when no heading is detected,
    so sparse-heading books (consistent font sizes) still produce a rich outline.
    """
    seen_full = set()
    seen_sub  = set()
    sections  = []
    ...
    for chunk in chunks:
        ...
        if heading and heading not in seen_full:
            ...
            # Fallback: derive a label from the first non-empty line of the chunk
            first_line = next(
