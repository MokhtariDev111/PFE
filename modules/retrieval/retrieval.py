...
import numpy as np
import sys
from modules.retrieval.query_expansion import expand_query_simple, deduplicate_results
...
def _load_reranker(model_name: str):
    ...
    if _GLOBAL_RERANKER is None:
        try:
            ...
            log.info(f"Loading reranker model '{model_name}' on CPU...")
            _GLOBAL_RERANKER = CrossEncoder(model_name, device="cpu")
            log.info("  ✔ Reranker loaded on CPU.")
...
class Retriever:
    ...
    def _load_db(self):
        import faiss
        from modules.ingestion.text_processing import TextChunk
    ...
    def _load_model(self):
        from modules.retrieval import embeddings
        if embeddings._GLOBAL_EMBED_MODEL is None:
            log.info(f"Loading embedding model '{self.model_name}'...")
            from sentence_transformers import SentenceTransformer
            embeddings._GLOBAL_EMBED_MODEL = SentenceTransformer(
                self.model_name, device=self.device
            )
        self.model = embeddings._GLOBAL_EMBED_MODEL
