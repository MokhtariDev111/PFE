...
def check_embedding_model() -> dict:
    ...
    try:
        from modules.retrieval.embeddings import _GLOBAL_EMBED_MODEL
...
def check_reranker() -> dict:
    ...
    if enabled:
        try:
            from modules.retrieval.retrieval import _GLOBAL_RERANKER
...
def check_cache() -> dict:
    """Check cache status."""
    try:
        from modules.core.llm_cache import cache_stats
        stats = cache_stats()
        stats["status"] = "active"
        return stats
    except ImportError:
...
def check_history() -> dict:
    ...
    try:
        from modules.core.history_store import load_history, HISTORY_FILE
...
def check_ocr() -> dict:
    ...
    try:
        from modules.ingestion.ocr import _EASYOCR_CACHE
